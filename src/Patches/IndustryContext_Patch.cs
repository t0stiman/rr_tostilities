...
// 		
// 		var damageFine = Mathf.FloorToInt( paymentOnArrival * Mathf.Lerp(0.0f, 0.75f, Mathf.InverseLerp(0.95f, 0.0f, car.Condition)));
// 		var timelyDeliveryBonus = 0;
// 		if (__instance._industry.HasActiveContract(now))
// 		{
// 			timelyDeliveryBonus = __instance._industry.Contract.Value.TimelyDeliveryBonus(Mathf.FloorToInt(now.DaysSince(waybill.Created)), paymentOnArrival);
// 		}
// 		
// 		int total = paymentOnArrival + num2 - num1;
// 		if (total == 0)
// 			return;
// 		this._industry.ApplyToBalance(total, Ledger.Category.Freight, count: 1, quiet: true);
// 		this._controller.AddCoalescedPaymentAnnouncement(car, this._industry, paymentOnArrival, new (int, string)[2]
// 		{
// 			(num2, "timely"),
// 			(-num1, "damage")
// 		}, total);
